...
import { uniform, frameId, pass, texture3D, time, screenCoordinate, texture, Fn, vec3, uv, normalView } from 'three/tsl';
...
async function init() {
    ...
    camera = new THREE.PerspectiveCamera(30, window.innerWidth / window.innerHeight, 0.025, 10);
    ...
    const causticEffect = Fn(() => {
        ...
        return causticColor.rgb.mul(causticIntensityUniform).mul(viewZ);
    })().toVar();
...
function setupTweakpane(textureLoader) {
    ...
    pane.addBinding(params, 'causticIntensity', { min: 1, max: 60, step: 1, label: 'Caustic Intensity' })
        .on('change', (ev) => { causticIntensityUniform.value = ev.value; });

    pane.addBinding(params, 'smokeAmount', { min: 0, max: 10, step: 0.1, label: 'Fog Density' })
        .on('change', (ev) => { smokeAmountUniform.value = ev.value; });

    pane.addBinding(params, 'bloomIntensity', { min: 0, max: 1, step: 0.02, label: 'Bloom' })
